...
class QUANTLHModel:
    ...
    def run3modes(self):
        ...
        # i = employment zone
        # j = residential zone
        # Ei = number of jobs at zone location
        # Aj = attractor of HH (household floorspace)
        # cij_mode = travel cost for "mode" (i.e. Car, bus, rail)
        # Modes: Car = 0, Bus = 1, Rail = 2
        ...
        while converged != True:
            ...
            # Debug:
            # commuter sum blocks
            TotalSij_Car = Sij[0].sum()
            TotalSij_bus = Sij[1].sum()
            TotalSij_rail = Sij[2].sum()
            TotalEi = self.Ei.sum()  # total jobs = pu+pr above
            # print("i= {0:d} beta_Car={1:.6f} beta_bus={2:.6f} beta_rail={3:.6f} cbar_pred_Car={4:.1f} cbar_pred_busr={5:.1f} cbar_pred_rail={6:.1f}"
            #         .format(iteration, Beta[0], Beta[1], Beta[2], CBarPred[0], CBarPred[1], CBarPred[2]))
            # print("TotalSij_Car={0:.1f} TotalSij_bus={1:.1f} TotalSij_rail={2:.1f} Total={3:.1f} ({4:.1f})"
            #       .format(TotalSij_Car, TotalSij_bus, TotalSij_rail, TotalSij_Car + TotalSij_bus + TotalSij_rail, TotalEi))
